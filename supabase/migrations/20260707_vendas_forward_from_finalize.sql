-- Allow Vendas to forward a CM from vendas_finalize to another department.
-- Some depts skip straight to Vendas before every needed dept has weighed in;
-- Vendas previously could only finalize or contest at that point. Now it can
-- also send the CM to a missing dept, same as any other dept's forward action.
-- Still blocked from forwarding outside vendas_finalize (Vendas never holds
-- the CM at 'open' stage under the normal flow, but create_cm could in theory
-- assign it there directly, so keep that path closed).
...
BEGIN
  ...
  SELECT d.slug INTO v_actor_slug FROM departments d WHERE d.id = v_actor_dept_id;
  IF v_actor_slug = 'vendas' AND v_cm.workflow_stage != 'vendas_finalize' THEN
    RAISE EXCEPTION 'Vendas can only forward from the vendas_finalize stage';
  END IF;

  SELECT d.slug INTO v_to_dept_slug FROM departments d WHERE d.id = p_to_dept_id;
